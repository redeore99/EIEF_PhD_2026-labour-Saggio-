...
\institute{\small{Labor Economics-EIEF}}
...
\vspace{0.05in}
\item Does $\beta$ have a causal interpretation? If not, is $\beta$ too big or too small?
\vspace{0.05in}
...
\begin{itemize}
\begin{small}
\item To investigate the returns to education, consider a static model of schooling investment, as in Card (1999), Handbook of Labor Economics (HOLE) chapter.
\medskip
\item Consider an individual deciding how long to stay in school, $S$.
\medskip
\item Schooling maps to earnings according to the differentiable function $y(S)$ where $y'(S)\equiv \dfrac{\partial y(S)}{\partial S}$ denotes the marginal return to schooling.
\medskip 
...
\medskip
\item Idea: individuals discount future earnings at rate $r$ and decide how many years to spend in school before earning some amount of money.
\medskip
...
\end{displaymath}
\item What is: $\beta - \gamma E[S_{i}^{*}]?$
\medskip
\item What is: $\gamma \dfrac{\sigma_{\alpha,r}}{\sigma^{2}_{r}}$
\medskip 
...
\vspace{0.05in}
\item IV estimates a local average treatment effect for a particular group of individuals known as compliers.
\end{small}
\end{itemize}
\end{small}
\end{itemize}
...
\vspace{0.05in}
\item Oreopoulos (2006) uses this change in an RD design, finds large returns
\vspace{0.05in}
...
\begin{frame}[plain] % plain removes header/footer
  \frametitle{Devereux and Hart 2010}
...
\vspace{0.05in}
\item DK is a great example of (credible) research based on a conditional independence assumption (CIA).
\end{itemize}
\end{footnotesize}
}
...
\begin{frame}{DK has also been replicated in Texas (w/ much better data) by Mountjoy and Hickman (2021)}
...
\begin{frame}
\centering
\includegraphics[page=1,scale=0.4]{aux/jesse.jpg}
\end{frame}

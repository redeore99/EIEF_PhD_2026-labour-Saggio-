...
believable literally, since technology surely receives shocks, but it is a
reasonable approximation if those shocks are small relative to supply movements.
...
(less monopsony power), which is directionally plausible. But the specific form is
...
firms versus bad firms, and produces a positive pay-amenity correlation, the
...
pushing positive). A positive measured covariance, namely the empirical pattern
that better-paying firms also have better amenities (the ``no compensating
differentials'' finding shown in class), therefore does \emph{not} reject
compensating differentials; it merely says that firm-quality dispersion
outweighs amenity-cost dispersion.
...
requires (i) data on $\bar V_j$ (unobserved, but exactly what Sorkin
